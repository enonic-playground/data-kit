...
describe('GET /binary (download)', () => {
    test('returns binary with correct contentType and headers', () => {
        ...
        expect(mockConn.getBinary).toHaveBeenCalledWith({ key: 'node-1', binaryReference: 'image.png' });
    });
    ...
    test('returns 404 for missing node', () => {
        ...
        const response = get({
            params: { repoId: 'my-repo', branch: 'master', key: 'nonexistent', binaryReference: 'image.png' },
        } as unknown as Request);
    ...
    test('returns 404 for missing binaryReference in _attachments', () => {
        ...
        const response = get({
            params: { repoId: 'my-repo', branch: 'master', key: 'node-1', binaryReference: 'missing.txt' },
        } as unknown as Request);
...
describe('GET /binary?info=true', () => {
    test('returns JSON metadata envelope', () => {
        ...
        const response = get({
            params: { repoId: 'my-repo', branch: 'master', key: 'node-1', binaryReference: 'image.png', info: 'true' },
        } as unknown as Request);
    ...
    test('returns info without label when absent', () => {
        ...
        const response = get({
            params: { repoId: 'my-repo', branch: 'master', key: 'node-1', binaryReference: 'doc.pdf', info: 'true' },
        } as unknown as Request);
        const body = parseBody(response);
    ...
    test('returns 404 for missing attachment in info mode', () => {
        ...
        const response = get({
            params: { repoId: 'my-repo', branch: 'master', key: 'node-1', binaryReference: 'nope.txt', info: 'true' },
        } as unknown as Request);
