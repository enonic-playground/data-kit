...
class Parser {
  ...
  private validateFunctionSemantics(
    ...
    if (!correctCategory) {
      const sourceLabel = isValue
        ? 'a value function'
        : isConstraint
          ? 'a constraint function'
          : 'an ORDER BY function';
      const destLabel =
        category === 'value' ? 'a value' : category === 'constraint' ? 'a constraint' : 'ORDER BY';
      throw new ParseError({
...
function formatArityMessage(name: string, expected: number | RangeArity, got: number): string {
  ...
  const range =
    expected.min === expected.max ? `${expected.min}` : `${expected.min}\u2013${expected.max}`;
