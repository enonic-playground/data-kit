...
function applyEventToTask(existing: TaskInfo | undefined, event: {
    type: string;
    timestamp: number;
...
export function useTaskProgress(taskId: string | undefined): void {
    ...
    useEffect(() => {
        ...
        const unsubscribe = subscribe((message: ServerMessage) => {
            ...
            queryClient.setQueryData<TaskInfo | undefined>(
                ['tasks', taskId],
                (existing) => applyEventToTask(existing, message),
            );

            const state = (message.data as TaskEventData).state
                ?? toTaskStateFromType(message.type);
