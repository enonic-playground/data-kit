...
const TasksPage = (): ReactElement => {
  ...
  const columns = [
      ...
      cell: (info) => (
        <div className="flex flex-col">
          ...
          {info.row.original.description !== '' && (
            <span className="text-muted-foreground text-xs">{info.row.original.description}</span>
          )}
        </div>
      ),
    }),
    columnHelper.accessor('state', {
      ...
      cell: (info) => (
        <Badge variant={TASK_STATE_VARIANT[info.getValue()]}>{info.getValue()}</Badge>
      ),
    }),
        ...
        return (
          <div className="flex min-w-40 flex-col gap-1">
            ...
            <span className="text-muted-foreground text-xs">
              {progress.info !== '' ? progress.info : `${progress.current}/${progress.total}`}
            </span>
          </div>
        );
      },
  ...
  return (
    <div data-component={TASKS_PAGE_NAME} className="flex flex-col">
      <div className="border-border bg-card flex h-10 shrink-0 items-center justify-between gap-1.5 overflow-x-auto border-b px-4">
        <span className="text-foreground font-mono text-xs font-medium">{t('nav.tasks')}</span>
...
export const Route = createFileRoute('/tasks')({
  loader: ({ context: { queryClient } }) => queryClient.ensureQueryData(tasksListQueryOptions()),
  component: TasksPage,
});
