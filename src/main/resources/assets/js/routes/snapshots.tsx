...
import { Camera, Ellipsis, KeyRound, Plus, RotateCcw, Trash2 } from 'lucide-react';
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>{t('snapshot.dialog.create.description')}</DialogDescription>
        </DialogHeader>
...
function getErrorContent(
  ...
  if (error.code === 'MANAGEMENT_API_ERROR') {
    return {
      ...
      description: (
        <Trans i18nKey="snapshot.error.managementApi.description" components={{ code: <Code /> }} />
      ),
    };
  }
  ...
  return (
    <div data-component={SNAPSHOTS_ERROR_NAME} className="flex flex-col">
      ...
      <div className="border-border bg-card flex h-10 shrink-0 items-center gap-1.5 overflow-x-auto border-b px-4">
        <span className="text-foreground font-mono text-xs font-medium">{t('nav.snapshots')}</span>
      </div>
      ...
      <EmptyState icon={KeyRound} title={title} description={description} />
    </div>
  );
};
...
export const Route = createFileRoute('/snapshots')({
  loader: ({ context: { queryClient } }) => queryClient.ensureQueryData(snapshotsQueryOptions()),
