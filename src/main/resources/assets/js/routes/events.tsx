...
const EventRow = ({ event }: EventRowProps): ReactElement => {
  ...
  return (
    <li data-component={EVENT_ROW_NAME} className="border-border border-b last:border-b-0">
      <button
        type="button"
        onClick={() => setExpanded((prev) => !prev)}
        className={cn(
          'flex w-full items-center gap-3 px-4 py-1.5 text-left',
          'hover:bg-row-hover focus-visible:ring-ring focus-visible:ring-1 focus-visible:outline-none',
        )}
      ...
      >
        ...
        <span className="text-foreground truncate font-mono text-xs">{summarize(event)}</span>
        {event.distributed && (
      ...
      {expanded && (
        <pre className="bg-muted/40 text-foreground overflow-x-auto px-4 py-2 font-mono text-xs">
...
const EventsPage = (): ReactElement => {
  ...
  const handleToggleCategory = useCallback((category: EventCategory, checked: boolean) => {
  ...
  const statusBadge = (
    <Badge
      variant={status === 'open' ? 'default' : status === 'connecting' ? 'outline' : 'destructive'}
    >
      {status === 'open'
  ...
  return (
    <div data-component={EVENTS_PAGE_NAME} className="flex h-full flex-col">
      ...
      <div className="border-border bg-card flex h-10 shrink-0 items-center justify-between gap-2 overflow-x-auto border-b px-4">
        <span className="text-foreground font-mono text-xs font-medium">{t('nav.events')}</span>
      ...
      {/* Toolbar */}
      <div className="border-border bg-card flex flex-wrap items-center gap-3 border-b px-4 py-2">
        <Button
          ...
          return (
            <div key={category} className="flex items-center gap-1.5">
              <Checkbox
                ...
                onCheckedChange={(checked) => handleToggleCategory(category, checked === true)}
              />
      ...
      {visibleEvents.length === 0 ? (
        <EmptyState
          icon={Activity}
          title={
            snapshot.events.length === 0 ? t('events.empty.title') : t('events.filtered.title')
          }
          description={
