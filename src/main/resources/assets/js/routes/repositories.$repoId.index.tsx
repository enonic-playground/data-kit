...
const branchIdSchema = z
  ...
  .regex(/^[a-z0-9][a-z0-9._-]*[a-z0-9]$/, 'branch.error.invalidChars');
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>{t('branch.dialog.create.description')}</DialogDescription>
        </DialogHeader>
...
const BranchListPage = (): ReactElement => {
  ...
  const { data: branches } = useSuspenseQuery(branchesQueryOptions(repoId));
  ...
  return (
    <div data-component={BRANCH_LIST_PAGE_NAME} className="flex flex-col">
      ...
      <div className="border-border bg-card flex h-10 shrink-0 items-center gap-1.5 overflow-x-auto border-b px-4">
        ...
        <ChevronRight className="text-text-dimmed size-2.5 shrink-0" />
        <span className="text-foreground font-mono text-xs font-medium">{repoId}</span>
      </div>
      ...
      <Table>
        ...
        <TableBody>
          {/* Back row */}
          <TableRow className="cursor-pointer" onClick={() => navigate({ to: '/repositories' })}>
